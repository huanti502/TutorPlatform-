...
builder.Services.AddMemoryCache();  // rate-limit AI theo user
...
builder.Services.AddScoped<TutorPlatform.Web.Services.ModerationService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        await db.Database.ExecuteSqlRawAsync(
            "ALTER TABLE \"TutorProfiles\" ADD COLUMN IF NOT EXISTS \"IntroVideoUrl\" text NULL;");
...
using (var paymentScope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        await pDb.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS "LearningGoals" (
                "Id" integer GENERATED BY DEFAULT AS IDENTITY,
                "StudentId" text NOT NULL,
                "Title" text NOT NULL,
                ...
                CONSTRAINT "PK_LearningGoals" PRIMARY KEY ("Id")
            );
            """);
        ...
        await pDb.Database.ExecuteSqlRawAsync("""
            ...
            """);
        if (!await pDb.BannedWords.AnyAsync())
        {
            foreach (var w in new[] { "đm", "đcm", "vcl", "lồn", "cặc", "địt", "óc chó", "mất dạy", "lừa đảo" })
                pDb.BannedWords.Add(new TutorPlatform.Core.Models.BannedWord { Word = w });
        ...
        // Bảng SupportTickets (liên hệ / yêu cầu hỗ trợ gửi admin)
        await pDb.Database.ExecuteSqlRawAsync("""
        ...
        // Nâng cấp bảng Payments: hỗ trợ nạp ví qua VNPay (BookingId nullable + Type + PayerUserId)
        await pDb.Database.ExecuteSqlRawAsync(
            "ALTER TABLE \"Payments\" ADD COLUMN IF NOT EXISTS \"Type\" text NOT NULL DEFAULT 'Booking';");
        await pDb.Database.ExecuteSqlRawAsync(
            "ALTER TABLE \"Payments\" ADD COLUMN IF NOT EXISTS \"PayerUserId\" text NULL;");
        await pDb.Database.ExecuteSqlRawAsync(
            "ALTER TABLE \"Payments\" ALTER COLUMN \"BookingId\" DROP NOT NULL;");

        // Bảng WithdrawalRequests (gia sư rút tiền từ ví)
        await pDb.Database.ExecuteSqlRawAsync("""
        ...
        // Bảng StudentReviews (gia sư đánh giá học viên — đánh giá 2 chiều)
        await pDb.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS "StudentReviews" (
                "Id" integer GENERATED BY DEFAULT AS IDENTITY,
                "BookingId" integer NOT NULL,
                "TutorProfileId" integer NOT NULL,
                "StudentId" text NOT NULL,
                ...
                "Comment" text NULL,
                "CreatedAt" timestamp with time zone NOT NULL DEFAULT now(),
                CONSTRAINT "PK_StudentReviews" PRIMARY KEY ("Id"),
                CONSTRAINT "UQ_StudentReviews_Booking" UNIQUE ("BookingId"),
                CONSTRAINT "FK_StudentReviews_Bookings" FOREIGN KEY ("BookingId") REFERENCES "Bookings" ("Id") ON DELETE CASCADE,
                CONSTRAINT "FK_StudentReviews_Users" FOREIGN KEY ("StudentId") REFERENCES "AspNetUsers" ("Id") ON DELETE CASCADE
            );
            """);

        // Bảng ClassRequests (lớp cần gia sư) + ClassApplications (đề nghị dạy)
        await pDb.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS "ClassRequests" (
                ...
                "BudgetPerSession" numeric NOT NULL DEFAULT 0,
                "Status" text NOT NULL DEFAULT 'Open',
                "CreatedAt" timestamp with time zone NOT NULL DEFAULT now(),
                CONSTRAINT "PK_ClassRequests" PRIMARY KEY ("Id"),
                CONSTRAINT "FK_ClassRequests_AspNetUsers_StudentId" FOREIGN KEY ("StudentId") REFERENCES "AspNetUsers" ("Id") ON DELETE CASCADE,
                CONSTRAINT "FK_ClassRequests_Subjects_SubjectId" FOREIGN KEY ("SubjectId") REFERENCES "Subjects" ("Id") ON DELETE SET NULL
            );
            """);
        await pDb.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS "ClassApplications" (
                "Id" integer GENERATED BY DEFAULT AS IDENTITY,
                "ClassRequestId" integer NOT NULL,
                "TutorProfileId" integer NOT NULL,
                "Message" text NULL,
                "ProposedRate" numeric NULL,
                "Status" text NOT NULL DEFAULT 'Pending',
                "CreatedAt" timestamp with time zone NOT NULL DEFAULT now(),
                CONSTRAINT "PK_ClassApplications" PRIMARY KEY ("Id"),
                CONSTRAINT "FK_ClassApplications_ClassRequests_ClassRequestId" FOREIGN KEY ("ClassRequestId") REFERENCES "ClassRequests" ("Id") ON DELETE CASCADE,
                CONSTRAINT "FK_ClassApplications_TutorProfiles_TutorProfileId" FOREIGN KEY ("TutorProfileId") REFERENCES "TutorProfiles" ("Id") ON DELETE CASCADE
            );
            """);
        await pDb.Database.ExecuteSqlRawAsync(
            "CREATE INDEX IF NOT EXISTS \"IX_ClassRequests_Status\" ON \"ClassRequests\" (\"Status\");");
        await pDb.Database.ExecuteSqlRawAsync(
            "CREATE INDEX IF NOT EXISTS \"IX_ClassApplications_ClassRequestId\" ON \"ClassApplications\" (\"ClassRequestId\");");
